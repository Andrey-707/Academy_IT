# Sudoku.tkinter 07.Обработка событий решение пазла.
...
Nums.grid(row=1, column=0, padx=(15,0), pady=(5,0), sticky='W')
...
ttk.Label(root, text='Count:', background=colors[0], foreground=colors[1]).\
    grid(row=1, column=2, padx=(5,5), pady=(5,0), sticky='W')

# поле ввода номера уровня
var_count = IntVar()
var_count.set(1)
edt_count = ttk.Entry(root, width=5, textvariable=var_count, justify=RIGHT, font=sym_font)
edt_count.grid(row=1, column=3, padx=(5,5), pady=(5,0), sticky='W')
