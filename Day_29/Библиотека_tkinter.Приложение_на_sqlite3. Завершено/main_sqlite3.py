...
def create_upper_legt():
    ...
    items = engine_sqlite3.get_all_expenses_items()
    l_choose = tk.Label(frame_add_form, text="● Виды затрат", font=fontExample, bg="#000", fg="#fff")
    f_choose = ttk.Combobox(frame_add_form, values=engine_sqlite3.get_all_expenses_items()["names"],
...
def create_upper_right():
    ...
    l_most_common_value = tk.Label(frame_statistic, text=engine_sqlite3.get_most_common_item(), font=fontExample, bg="#000", fg="#fff")
    l_exp_item_text = tk.Label(frame_statistic, text="● Самая дорогая покупка", font=fontExample, bg="#000", fg="#fff")
    l_exp_item_value = tk.Label(frame_statistic, text=engine_sqlite3.get_most_exp_item(), font=fontExample, bg="#000", fg="#fff")
    l_exp_month_text = tk.Label(frame_statistic, text="● Месяц самой дорогой покупки", font=fontExample, bg="#000", fg="#fff")
    l_exp_month_value = tk.Label(frame_statistic, text=engine_sqlite3.get_most_exp_month(), font=fontExample, bg="#000",fg="#fff")
    l_exp_day_text = tk.Label(frame_statistic, text="● День самой дорогой покупки", font=fontExample, bg="#000", fg="#fff")
    l_exp_day_value = tk.Label(frame_statistic, text=engine_sqlite3.get_most_exp_day(), font=fontExample, bg="#000", fg="#fff")
...
def form_submit():
    ...
    id_ = engine_sqlite3.get_id() + 1 # номер id = последний нормер +1
    amount = float(f_amount.get()) # сумма в формате числа с плавающей точкой
    payment_date = engine_sqlite3.get_timestamp_from_string(f_date.get()) # дата YYYY-mm-dd
    expense_id = items["accordance"][f_choose.get()] # items см. в функции create_upper_legt()

    with sqlite3.connect("database.db") as db:
        cursor = db.cursor()
        # !!! В sqlite3 ИСПОЛЬЗУЕТСЯ ПЕРЕДАЧА ЗНАЧЕНИЙ В ФОРМАТЕ ? !!!
        query = "INSERT INTO payments(id, amount, payment_date, expense_id) VALUES (?, ?, ?, ?);" # добавление данных с использованием переменных
...
def form_cancel():
    ...
    id_ = engine_sqlite3.get_id()
    with sqlite3.connect("database.db") as db:
        cursor = db.cursor()
        # !!! В sqlite3 ИСПОЛЬЗУЕТСЯ ПЕРЕДАЧА ЗНАЧЕНИЙ В ФОРМАТЕ ? !!!
        delete = "DELETE FROM payments WHERE payments.id = ?;" # удаление данных с использованием переменной
...
def create_lower():
    ...
    for row in engine_sqlite3.get_table_data():
        ...
        for i, j in enumerate(row):
            if j not in rrow :
                if i == 3:
                    rrow.append(engine_sqlite3.get_date(j))
                else:
                    rrow.append(j)
            elif i != j:
                rrow.append(j)
...
def main():
    ...
    window = tk.Tk()
    window.title("Purse (sqlite3)") # шапка
