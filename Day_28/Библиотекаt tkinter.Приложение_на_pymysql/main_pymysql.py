...
# !!! Поскольку в модуле engine_pymysql используется pymysql и База Данных pymysql разворачивается на
# сервере, то для работы программы необходимо включить Open Server Panel, затем запустить (справа в трее) !!!
...
l_most_common_value = tk.Label(frame_statistic, text=engine_pymysql.get_most_common_item(), font="Helvetica 12 bold")
l_exp_item_text = tk.Label(frame_statistic, text="● Самая дорогая покупка", font="Helvetica 12")
l_exp_item_value = tk.Label(frame_statistic, text=engine_pymysql.get_most_exp_item(), font="Helvetica 12 bold")
...
# for row in engine_pymysql.get_table_data(): # принудительно увеличить строки таблицы, чтобы посмотреть как работает Scrollbar
#     table.insert('', tk.END, values=row)
...
table["yscrollcommand"] = scroll_pane.set
